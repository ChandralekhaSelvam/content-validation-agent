...
Flow:
    router → (generate_from_topic | extract_doc → rewrite_from_doc) → validate
    validate → (revise → validate)*   [loop, bounded by max_iterations]
    validate → human_review → END     [if flagged or loop exhausted]
    validate → finalize → END         [clean pass, no flags]
"""
...
from nodes.finalize import finalize, human_review
...
def build_graph():
    ...
    graph.add_edge("revise", "validate")  # the loop
